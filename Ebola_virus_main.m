% Mechanics
% ex11 of Shampine
% Initialization mex Jac_create.c -R2018a %compile mex file
pause(0.0001)
clear ; close all; clc
...
t0=0; %initial time
tf=1; %final time
u0=[.1;.1;.1;.1;.1]; %initial conditions
%
f=@(t,x) Eboladisease(t,x); %ODE system
derf=@(t,x) eboladisease_jac(t,x); %Jacobian (For RPNN the Jacobian need to be transposed)
%
nEq=max(size(u0));
%Mass Matrix
Mmass=diag([1,1,1,1,1]);
%
nEq=max(size(u0)); %number of equations
var_name={'S(t)','E(t)','I(t)','Q(t)','R(t)'}; %name of variables
%
